param([string]$Artifact = "$PSScriptRoot/atoms-staging-preview-gateway-private-live-session-smoke-v18.ps1")
...
function Check($Condition, [string]$Message) {
    if (-not $Condition) { throw "V18 OFFLINE ASSERTION FAILED: $Message" }
    $script:Checks++
}
...
$script:CaseRoot = Join-Path ([IO.Path]::GetTempPath()) ("atoms-v18-offline-" + [Guid]::NewGuid().ToString("N"))
...
function Success-Tokens {
    $names = @("PAYLOAD_CLEAN_OK", "PACKAGE_OK", "REDIS_WRITE_OK", "TTL_SET_OK", "HTTP_FORWARD_OK", "WS_FORWARD_OK", "TTL_EXPIRED_OK", "REDIS_CLEANUP_OK", "CLIENT_CLOSED_OK", "LIVE_OK")
    return @($names | ForEach-Object { "ATOMS_PVE2E_$($_):$($script:RunId)" }) -join [Environment]::NewLine
}
    ...
    switch -Wildcard ($prefix) {
        ...
        "containerapp exec*" {
            ...
            if ($script:CaseName -ceq "failure-http") {
                return @{ ExitCode = 1; Stderr = "remote exit"; Stdout = "ATOMS_PVE2E_PAYLOAD_CLEAN_OK:$($script:RunId)`nATOMS_PVE2E_PACKAGE_OK:$($script:RunId)`nATOMS_PVE2E_REDIS_WRITE_OK:$($script:RunId)`nATOMS_PVE2E_TTL_SET_OK:$($script:RunId)`nATOMS_PVE2E_REDIS_CLEANUP_OK:$($script:RunId)`nATOMS_PVE2E_CLIENT_CLOSED_OK:$($script:RunId)`nATOMS_PVE2E_FAILED_HTTP_FORWARD:$($script:RunId)" }
            }
            if ($script:CaseName -ceq "cleanup-unconfirmed") {
                return @{ ExitCode = 1; Stderr = "remote exit"; Stdout = "ATOMS_PVE2E_PAYLOAD_CLEAN_OK:$($script:RunId)`nATOMS_PVE2E_PACKAGE_OK:$($script:RunId)`nATOMS_PVE2E_REDIS_WRITE_OK:$($script:RunId)`nATOMS_PVE2E_FAILED_CLEANUP:$($script:RunId)" }
            }
            if ($script:CaseName -ceq "signal-absent") { return @{ ExitCode = 0; Stdout = ""; Stderr = "connected" } }
            if ($script:CaseName -ceq "signal-incomplete") { return @{ ExitCode = 0; Stdout = "ATOMS_PVE2E_PAYLOAD_CLEAN_OK:$($script:RunId)"; Stderr = "" } }
            ...
            if ($script:CaseName -ceq "unrelated-drift") {
                ...
                return @{ ExitCode = 1; Stdout = "ATOMS_PVE2E_FAILED_HTTP_FORWARD:$($script:RunId)"; Stderr = "" }
            }
...
function Reset-Case([string]$Name) {
    ...
    $script:RunId = "0123456789abcdef0123456789abcdef" # deterministic offline proof fixture
    $script:RevisionPrefix = "pve2e-" + $script:RunId.Substring(0, 12)
    $script:StageFile = "/tmp/atoms-pve2e-$($script:RunId).b64"
...
foreach ($name in $caseNames) {
    ...
    if ($name -in $successCases) {
        ...
        if ($name -ceq "failure-http") { Check (($output -join " ") -match "ATOMS_LIVE_PROBE_FAILED_HTTP_FORWARD") "Live phase failure is exact" }
...
$script:StateApp.properties.template.revisionSuffix = "pve2e-$($attempt.Substring(0,12))-a"
$script:StateApp.properties.latestRevisionName = "$PreviewName--$($script:StateApp.properties.template.revisionSuffix)"
$script:StateApp.properties.latestReadyRevisionName = $script:StateApp.properties.latestRevisionName
...
$script:Apply = $false
$output = @(Invoke-LiveSessionSmoke *>&1)
Check ($script:ExitStatus -eq 0 -and $script:CommitSucceeded) "Exact v18 checkpoint recovers interrupted min=1"
...
Check (@($carrier.Commands).Count -eq 2 -and $carrier.MaxCommandLength -le 1800) "Carrier uses two bounded staging execs"
Check (-not $carrier.FinalCommand.Contains("ATOMS_PVE2E_") -and
...
Write-Host "V18 OFFLINE POWERSHELL TESTS PASSED: $($script:Checks) assertions; $($caseNames.Count) apply/preflight cases + recovery. No Azure request was made."
