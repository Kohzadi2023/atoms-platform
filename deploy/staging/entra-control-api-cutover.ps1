...
$ExpectedIssuer = "https://atomsstaging91ce9.ciamlogin.com/1dda8889-b5fc-43eb-857b-b1549e4b82c3/v2.0"
$ExpectedJwksUrl = "https://atomsstaging91ce9.ciamlogin.com/1dda8889-b5fc-43eb-857b-b1549e4b82c3/discovery/v2.0/keys"
...
function Assert-VerifiedEntraState {
    ...
    if ([string]$apps.subscriptionId -eq $ForbiddenSubscriptionId -or [string]$flow.subscriptionId -eq $ForbiddenSubscriptionId) {
        throw "Refusing to continue: a state file references the forbidden legacy subscription."
    }
    if ([string]$apps.subscriptionId -ne $ExpectedSubscriptionId -or [string]$flow.subscriptionId -ne $ExpectedSubscriptionId) {
        throw "Unexpected subscription in verified Entra state."
    }
    if ([string]$apps.tenantId -ne $ExpectedTenantId -or [string]$flow.tenantId -ne $ExpectedTenantId) {
...
try {
    ...
    Write-Host "Atoms Staging Control API Entra Cutover v1" -ForegroundColor DarkGray

    Write-Step "Lock Azure CLI to the dedicated Atoms-Staging subscription"
    $az = Get-AzCommand
    & $az account set --subscription $ExpectedSubscriptionId
    if ($LASTEXITCODE -ne 0) { throw "Could not select Atoms-Staging subscription." }

    $account = (Invoke-AzCapture -Arguments @("account", "show", "--output", "json", "--only-show-errors")) | ConvertFrom-Json
    ...
    if ([string]$app.name -ne $ControlApiName -or [string]$app.resourceGroup -ne $ExpectedResourceGroup) {
        throw "Unexpected Container App identity."
    }

    $fqdn = [string]$app.properties.configuration.ingress.fqdn
    if ([string]::IsNullOrWhiteSpace($fqdn)) { throw "Control API ingress FQDN is unavailable." }

    $currentEnv = @($app.properties.template.containers[0].env)
    $authRequired = @($currentEnv | Where-Object { [string]$_.name -eq "AUTH_REQUIRED" }) | Select-Object -First 1
    ...
    Write-Step "Create a new Container Apps revision with verified Entra OIDC settings"
    & $az containerapp update `
        ...
        --set-env-vars `
            "AUTH_REQUIRED=true" `
            "AUTH_ISSUER_URL=$ExpectedIssuer" `
            "AUTH_AUDIENCE=$ExpectedApiClientId" `
            "AUTH_JWKS_URL=$ExpectedJwksUrl" `
            "AUTH_ALLOWED_ALGORITHMS=RS256" `
        --output none `
        --only-show-errors

    if ($LASTEXITCODE -ne 0) { throw "Control API Entra environment update failed." }
    Write-Ok "Verified Entra OIDC settings applied"
    ...
    foreach ($entry in $expected.GetEnumerator()) {
        $actual = @($updatedEnv | Where-Object { [string]$_.name -eq [string]$entry.Key }) | Select-Object -First 1
    ...
    if ($ready.StatusCode -ne 200) { throw "Readiness endpoint returned HTTP $($ready.StatusCode)." }

    Write-Ok "Control API is ready with verified Entra OIDC settings"
    Write-Host ""
    ...
    Write-Host "  Issuer   : $ExpectedIssuer"
    Write-Host ""
    Write-Host "Next gate: rebuild the Web image with the verified NEXT_PUBLIC_ENTRA_* build arguments, deploy it, acquire two real customer tokens through the SPA, then run the Entra staging identity smoke." -ForegroundColor Cyan
}
finally {
    try {
        $az = Get-AzCommand
