...
test("the content approval is expected only when a content agent is part of the run", () => {
  assert.deepEqual(expectedApprovalScopes("GENERAL"), ["plan", "content"]);
  assert.deepEqual(expectedApprovalScopes("CLIENT_PORTAL"), ["plan"]);
});
