...
- **Smoke test is capability-based.** `scripts/smoke-staging-authenticated.mjs` no longer hardcodes eight agents. `PROJECT_TYPE_AGENTS` moved to `@atoms/contracts` so the worker and the smoke share one definition; the smoke takes `--project-type` (default `GENERAL`), checks coverage through `scripts/project-capability-coverage.mjs` (#117, capability names kept in step with the contracts table by a drift test), requires that no agent the type does not need produced output, and expects the content approval only when a content agent is part of the run. The check `seven_agent_artifacts` is now `required_agent_artifacts` (also in the recovery rehearsal's required list).
